...
function Game(getSong: SongType) {
  ...
  const getRandomNumber = useCallback(() => {
    ...
    if (scores.askedRhymes.includes(randomNumberGenerate.toString()))
      getRandomNumber();
  ...
  const resetData = () => {
    ...
    localStorage.setItem(
      ...
      validatedScore
        ? `${validatedScore},${
            scores.goodAnswers
          }/${pourcentageGoodAnswers}/${Date.now()}`
        : `${scores.goodAnswers}/${pourcentageGoodAnswers}/${Date.now()}`
    );
  ...
  if (
    ...
    console.log(getSong.lyrics[RandomNumber + 1]);

  return (
    <div className="flex justify-center">
      <div className="flex flex-col gap-5 w-4/5 mt-10">
        <div className="text-center text-xl">
          {scores.step === 10 ? "Tu as finis !" : song}
        </div>
        <Input
          type="text"
