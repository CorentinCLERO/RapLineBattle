...
function Page() {
  ...
  const handleSubmit = async () => {
    ...
    const artist = (document.querySelector('input[placeholder="Nom de l\'artiste"]') as HTMLInputElement).value;
    const songName = (document.querySelector('input[placeholder="Nom de la musique"]') as HTMLInputElement).value;
    const lyrics = (document.querySelector('textarea[placeholder="Paroles de la musique (Les retours à la ligne marqueront la différence entre chaque phrase)"]') as HTMLTextAreaElement).value;
  
    try {
      await schema.validate({ artist, songName, lyrics }, { abortEarly: false });
